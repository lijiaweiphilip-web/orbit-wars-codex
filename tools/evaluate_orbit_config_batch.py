...
from orbitwars.heuristics import HeuristicParams, make_agent
...
def evaluate_config(params_payload: dict, seeds: list[int]) -> dict[str, float | int]:
...
def _evaluate_entry(payload: tuple[dict, list[int]]) -> dict:
    entry, seeds = payload
    metrics = evaluate_config(entry["params"], seeds)
    ...
    with result_path.open("w", encoding="utf-8") as out:
        if args.workers <= 1:
            for entry in entries:
                row = _evaluate_entry((entry, seeds))
                out.write(json.dumps(row, sort_keys=True) + "\n")
                results.append(row)
        else:
            with ProcessPoolExecutor(max_workers=args.workers) as pool:
                futures = [pool.submit(_evaluate_entry, (entry, seeds)) for entry in entries]
